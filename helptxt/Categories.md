...
Best Laps|This column only applies in 'Best n laps' Time Trial mode.  Enter the number of laps (which should be fewer than the number of Race Laps) to count towards the rider's final time/speed and position.  For example, to count the best one of three laps, you'd set Race Laps to '3' and Best Laps to '1'.

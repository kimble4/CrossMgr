...
if 'MAC' in wx.Platform:
	...
	if os.path.isdir( os.path.join(topdirName, 'HPVMgrImages') ):
		dirName = topdirName
	else:
	...
	if os.path.basename(dirName) in ['library.zip', 'MainWin.exe', 'HPVMgr.exe']:
		dirName = os.path.dirname(dirName)
	if 'HPVMgr?' in os.path.basename(dirName):
		dirName = os.path.dirname(dirName)
	if not os.path.isdir( os.path.join(dirName, 'HPVMgrImages') ):
		dirName = os.path.dirname(dirName)

	if os.path.isdir( os.path.join(dirName, 'HPVMgrImages') ):
		pass
	elif os.path.isdir( '/usr/local/HPVMgrImages' ):
		dirName = '/usr/local'

imageFolder = os.path.join(dirName, 'HPVMgrImages')
htmlFolder = os.path.join(dirName, 'HPVMgrHtml')
helpFolder = os.path.join(dirName, 'HPVMgrHtmlDoc')
helpIndexFolder = os.path.join(dirName, 'HPVMgrHelpIndex')
...
lang = os.environ.get('HPVMgrLanguage', None)
...
def initTranslation():
	...
	if not initTranslationCalled or (lang and not lang.startswith('en')):
		...
		try:
			gettext.install('messages', os.path.join(dirName,'HPVMgrLocale'), unicode=1)
		except Exception:
			gettext.install('messages', os.path.join(dirName,'HPVMgrLocale'))
		
		# Try to use a translation matching the user's language.
		try:
			translation = gettext.translation('messages', os.path.join(dirName,'HPVMgrLocale'), languages=[lang[:2]])
			translation.install()

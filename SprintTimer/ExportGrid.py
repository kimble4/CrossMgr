...
brandText = _('Powered by BHPC SprintTimer (https://github.com/kimble4/CrossMgr)')
def getBrandText():
...
class ExportGrid:
	...
	def setResultsOneList( self, category = None, getExternalData = True ):
		...
		results = race.getSprintResults(category)
		if not results:
			print('no results')
			return
		...
		catData.append( '{} {} {}'.format(_('Sprint of'), race.sprintDistance, _('metres')) )
		...
		if getExternalData:
			...
			infoFields = ReportFields
		else:
			infoFields = []
		
		infoFieldsPresent = set( infoFields ) & set( externalInfo[leaderBib].keys() )
		infoFieldsPresent.add('Category')  # Fudge this as it's mapped to EventCategory
		...
		if race.distanceUnit == Model.Race.UnitKm:
			self.colnames.append( _('kph') )
		elif race.distanceUnit == Model.Race.UnitMiles:
			self.colnames.append( _('mph') )
		else:
			self.colnames.append( _('m/s') )
			
		self.colnames.append( _('Attempts') )
		...
		rrFields = (['pos', 'num'] +
					infoFields +
					['sprintTime', 'sprintSpeed', 'attempts']
		...
		for col, f in enumerate( rrFields ):
			if f in ('pos'):
				...
				for row, bibSprintDicts in enumerate(results):
					if bibSprintDicts is not None:
						if bibSprintDicts[1] is not None:
							if 'sprintTime' in bibSprintDicts[1][0]:
								data[col].append( '{:.3f}'.format(bibSprintDicts[1][0]['sprintTime']))
			elif f in ('sprintSpeed'):
				for row, bibSprintDicts in enumerate(results):
					if bibSprintDicts is not None:
						if bibSprintDicts[1] is not None:
							...
							if 'sprintDistance' in sprintDict:
								speed = float(sprintDict['sprintDistance']) / float(sprintDict['sprintTime'])
								if race.distanceUnit == Model.Race.UnitKm:
									out =  str('{:.2f}'.format(speed*3.6))
								elif race.distanceUnit == Model.Race.UnitMiles:
									out = str('{:.2f}'.format(speed*2.23694))
								else:
									out = str('{:.2f}'.format(speed))
							else:
								out = ''
							data[col].append( out )
			elif f in ('attempts'):
				for row, bibSprintDicts in enumerate(results):
					if bibSprintDicts is not None:
						if bibSprintDicts[1] is not None:
							data[col].append( len(bibSprintDicts[1]) )
				...
				for row, bibSprintDicts in enumerate(results):
					#print(f + ' for bib ' + str(bibSprintDicts[0]))
					try:

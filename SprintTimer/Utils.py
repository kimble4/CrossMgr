...
if 'MAC' in wx.Platform:
	...
	if os.path.isdir( os.path.join(topdirName, 'SprintTimerImages') ):
		dirName = topdirName
	else:
	...
	if os.path.basename(dirName) in ['library.zip', 'MainWin.exe', 'SprintTimer.exe']:
		dirName = os.path.dirname(dirName)
	if 'SprintTimer?' in os.path.basename(dirName):
		dirName = os.path.dirname(dirName)
	if not os.path.isdir( os.path.join(dirName, 'SprintTimerImages') ):
		dirName = os.path.dirname(dirName)

	if os.path.isdir( os.path.join(dirName, 'SprintTimerImages') ):
		pass
	elif os.path.isdir( '/usr/local/CrossMgrImages' ):
		dirName = '/usr/local'

imageFolder = os.path.join(dirName, 'SprintTimerImages')
htmlFolder = os.path.join(dirName, 'SprintTimerHtml')
helpFolder = os.path.join(dirName, 'SprintTimerHtmlDoc')
helpIndexFolder = os.path.join(dirName, 'SprintTimerHelpIndex')
...
lang = os.environ.get('SprintTimerLanguage', None)
...
def initTranslation():
	...
	if not initTranslationCalled or (lang and not lang.startswith('en')):
		...
		try:
			gettext.install('messages', os.path.join(dirName,'SprintTimerLocale'), unicode=1)
		except Exception:
			gettext.install('messages', os.path.join(dirName,'SprintTimerLocale'))
		
		# Try to use a translation matching the user's language.
		try:
			translation = gettext.translation('messages', os.path.join(dirName,'SprintTimerLocale'), languages=[lang[:2]])
			translation.install()
...
def getHomeDir( appName='SprintTimer' ):
